...
# We import TimeKeeper in the test, but it might not exist yet -> RED
try:
...
class TestCircadianLogic(unittest.TestCase):
    ...
    def test_rollover_pre_5am(self):
        ...
        if not TimeKeeper: self.skipTest("No TimeKeeper")
        
        # Mock datetime to be 2026-01-19 02:00:00
        mock_now = datetime.datetime(2026, 1, 19, 2, 0, 0)
        
        with patch('engine.modules.time_keeper.datetime') as mock_dt:
